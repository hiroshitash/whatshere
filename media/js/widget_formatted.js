 /*                                                                                                                                                                                                                                          
  * (c) 2010 Chad Etzel - TweetGrid.com                                                                                                                                                                                                       
 ...
 var jtw_searchfunc = function (arr) {
   if (jtw_search != jtw_current_search) 
     return false;

   clearTimeout(jtw_lock_timeout_timer);
   var ret = "<div style='font-size:10px;line-height:11px;margin-bottom:3px;'>Brought to you by:<br/><b><a href='http://tweetfireapp.com/?r=tgw' target='_blank'>TweetFire: A blazing fast twitter client!</a></b></div><hr style='border:0;color:#aaa;background-color:#aaa;height:1px;width:90%;'/>";
   ret = ret + jtw_pre_html;
   var i = 0;
   var bg;
   var wasnew = 0;
   var now = new Date();
   ...
   for (i = 0; i < jtw_num_tweets; i++) {
     ...
     ret = ret + '>';
     if (!jtw_hide_img) {
       ret = ret + '<img src=' + img + ' height=' + jtw_img_size + ' width=' + jtw_img_size + ' align=left style="padding:2px;">';
     }
     ret = ret + '<b><a style="' + jtw_tweet_linkcolor + 'text-decoration:none;" target="_new" href=http://twitter.com/' + user + '>' + user + '</a></b>: ' + tweet + '<br><span class="jtw_tstamp" style="font-size:10px;">' + tstamp;
     ret = ret + '</span>';
   ...
   if (num_newtweets == 0) {
       document.getElementById(jtw_divname).innerHTML = "";
   } else {
       document.getElementById(jtw_divname).innerHTML = ret;
   }
   if (typeof (jQuery) == 'function' && typeof ($) == 'function') {
     $('#' + jtw_divname + ' .jtw_new_tweet').fadeIn("slow");
   }
   jtw_cleanup();
 };
 jtw_refresh_clbk = function (i) {
   ...
   jtw_search_geocode = jtw_settings[i]['jtw_search_geocode'];
 ...
 if (!jtw_divname) jtw_divname = 'tweet_widget';
 ...
 if (!jtw_search_geocode) jtw_search_geocode = '37.787708,-122.41005,1mi';
 if (!jtw_num_tweets) jtw_num_tweets = 20;
 if (!jtw_pre_html) jtw_pre_html = '';
 ...
 jtw_settings[jtw_idx]['jtw_search_geocode'] = jtw_search_geocode;
 ...
 tw_frame_div = document.getElementById('tweet_frame');
 tw_widget_div = document.getElementById(jtw_settings[jtw_idx]['jtw_divname']);
 tw_frame_div.style = '"width:' + jtw_width + ';height:' + jtw_height + ';' + jtw_center_widget + ';padding:0px;margin:0px;' + '"';
 tw_widget_div.style = '"' + jtw_scroll + '' + jtw_center_widget + 'width:auto;height:' + jtw_height + ';background:' + jtw_widget_background + ';' + 'border:' + jtw_widget_border + ';' + jtw_widget_style_misc + '"'

/*
 target_div.style.width = jtw_width;
 target_div.style.height = jtw_height;
 target_div.style.padding = '0px';
 target_div.style.margin = '0px';
*/
 
 tw_widget_div.innerHTML = '<a href="http://whos.amung.us/stats/3iffs50u4co5/"><img src="http://whos.amung.us/widget/3iffs50u4co5.png" width="1" height="1" border="0" title="" /></a>';

 if (jtw_search != '-') {
    //tw_widget_div.innerHTML += '<script src="http://search.twitter.com/search.json?callback=jtw_searchfunc&' + jtw_tweet_lang + 'q=' + jtw_urlencode(jtw_search) + '&rpp=' + jtw_num_tweets + '&geocode=' + jtw_search_geocode + '&since_id=0" type="text/javascript"></script>';

     //alert('http://search.twitter.com/search.json?callback=jtw_searchfunc&' + jtw_tweet_lang + 'q=' + jtw_urlencode(jtw_search) + '&rpp=' + jtw_num_tweets + '&geocode=' + jtw_search_geocode + '&since_id=0');
    $.getScript('http://search.twitter.com/search.json?callback=jtw_searchfunc&' + jtw_tweet_lang + 'q=' + jtw_urlencode(jtw_search) + '&rpp=' + jtw_num_tweets + '&geocode=' + jtw_search_geocode + '&since_id=0');
 }
...
/*
 ...
 document.write('</div>');
 if (jtw_search != '-') {
   document.write('<script src="http://search.twitter.com/search.json?callback=jtw_searchfunc&' + jtw_tweet_lang + 'q=' + jtw_urlencode(jtw_search) + '&rpp=' + jtw_num_tweets + '&geocode=' + jtw_search_geocode + '&since_id=0" type="text/javascript"></script>');
 } else {} if (jtw_trk != "done") {
   try {
     if (typeof (_gat) != "object") {
       var tgwgaJsHost = (("https:" == document.location.protocol) ? "https://ssl." : "http://www.");
       document.write(unescape("%3Cscript src='" + tgwgaJsHost + "google-analytics.com/ga.js' type='text/javascript'%3E%3C/script%3E"));
       document.write(unescape("%3Cscript type='text/javascript'%3E"));
       document.write('try{var tgwTracker = _gat._getTracker("UA-897151-16");');
       document.write('tgwTracker._trackPageview("/_/" + location.toString());}catch(err){}');
       document.write(unescape("%3C/script%3E"));
     }
   } catch (err) {}
   document.write('<img src="http://pixel.quantserve.com/pixel/p-1fbjDQkLK5mdY.gif" style="display: none;" border="0" height="1" width="1" alt="Quantcast"/>');
   jtw_trk = "done";
 }
*/